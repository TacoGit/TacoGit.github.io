...
setInterval(function() {
  if (document.getElementById("bpm").innerHTML == "V1.0")
    return;

  var bpm = document.getElementById("bpm").innerHTML
  var numberified_bpm = Number(bpm);

  if (numberified_bpm >= 145) {
    //rainthroughnum = 40;
    rainnum = 100;
    speedRainTrough = 30;
  }

  if (numberified_bpm.inRange(110, 145)) {
    // rainthroughnum = 20;
    rainnum = 80;
    speedRainTrough = 25;
  }

  if (numberified_bpm.inRange(70, 110)) {
    //  rainthroughnum = 10;
    rainnum = 50;
    speedRainTrough = 15;
  }

  if (numberified_bpm < 70) {
    //  rainthroughnum = 40;
    rainnum = 25;
    speedRainTrough = 10;
  }

  console.log(numberified_bpm)
}, 1500)
...
var playingrain = false
var audieo = new Audio("rain/rain_one.mp3");

function playRain() {
  audieo.volume = 0.25
  if (playingrain) {
    audieo.pause();
    document.getElementById("rrr").innerHTML = "Play Rain Sound"
    playingrain = false;
  } else {
    audieo.play();
    document.getElementById("rrr").innerHTML = "Stop Rain Sound"
    playingrain = true;
  }
}
...
function nextRain() {
  ...
  audieo.pause()
  if (aud == 0) {
    audieo = new Audio("rain/rain_two.mp3")
    console.log("test")
    aud = aud + 1
  } else
    if (aud == 1) {
      audieo = new Audio("rain/rain_one.mp3")
      console.log("r")
      aud = aud + 1
    } else
      if (aud == 2) {
        audieo = new Audio("rain/rain_three.mp3")
        console.log("r")
        aud = aud + 1
      } else
        if (aud == 3) {
          audieo = new Audio("rain/rain_four.mp3")
          console.log("r")
          aud = 0
        }
  audieo.load()
